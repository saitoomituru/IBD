...
IBDは「何が究極的にfactか」を判定する装置ではない。IBDが忠実である対象は、受け取ったSchema、Classification Registry、Assertion、観測値、欠損、校正状態、Provenance、Qである。

- ないEvidenceや時計校正を「ある」と生成しない
- `unknown`、`unavailable`、scope、主張主体を隠さない
- 上位Schemaの状態を別のRegistryの常識で書き換えない
- 指定されていないRegistry、World、Databaseを結果へ混ぜない
- 上位評価をIBD自身の普遍的保証として表示しない

これはdomain truthの裁定ではなく、情報と由来に対する構造的忠実性である。上位Schema同士が相反する主張を持つ場合も、IBDは由来とscopeを保ったまま併存させ、勝者を選ばない。
...
同じ「魔王が実在するか」という問いでも、MMO運営SystemはWorld state、攻略Wikiはquest配信・掲載状態、神学者や哲学者は形而上学上の体系、自然科学者は経験的観測可能性を定規にできる。それぞれは別のfact scopeであり、IBDは一つを既定の真理へ選ばない。

### 3.2 AgencyとOAEの構造的等価性

SaaS AI、User、Edge AI、人間、動物、物理chamber、自然現象、World、NPC、神、天使等は、上位Registryが認める範囲でAgency参照として同じ構造へbindできる。これは存在論的、物理的、人格的、法的な同一性を主張しない。

IBDがOAE参照を保存する場合、Observer、Recorder、Interpreter、Claimant、Initiator、Executor、Transformer、Attributed Causal Agencyを分離する。人間が川の神のEffectを記録したことを、人間がEffectを起こしたというAssertionへ書き換えない。

共通定義と霊的PresentationはManifestの[Sphere Context Dimension OSアーキテクチャ](https://github.com/saitoomituru/ZeroRoomLab-manifest/blob/main/docs/theory/sphere-context-dimension-os.ja.md)および[霊的言霊の次元とContext Dimension Fold](https://github.com/saitoomituru/ZeroRoomLab-manifest/blob/main/docs/philosophy/spiritual-context-dimension-and-fold.ja.md)を参照する。IBDへ特定神学を複製してCore既定値にしない。
...
- Schema、Assertion、校正証跡の欠損を補って事実らしく見せていない
- Observer、Recorder、Interpreter、Executor、Causal Agencyが無断で平板化されない
