...
同じ4DだがDimension IDが異なる二つのFoldを用意する。

1. D数だけでは互換と判定しない
2. Access Mapだけを実行済みreceiptとみなさない
3. cross-Fold変換にMapping FAMとTransformerを要求する
4. Transformer不明時に架空Agencyを生成しない
5. Run TraceとOAE参照を分離する
6. Source Event、観測OAE、神学解釈OAE、安全工学解釈OAEを非破壊で並存させる

## 9. Experiment G: SsC calibrationと逆射影

異なるmetric、score direction、分布を持つ人工local retrieval runを作る。

1. raw scoreを直接sortしない
2. calibration dataset fingerprintとrevisionを要求する
3. local scoreからmeta SINへの単調性とuncertaintyを検査する
4. target SIN範囲からlocal thresholdへの逆射影可能性を検査する
5. `sin(θ) = sin(π - θ)`によるsector曖昧性を検出する
6. `atan`等の有界写像が分布校正そのものではないことを検査する
7. 未校正storeを`inverse-unavailable`として保持する
